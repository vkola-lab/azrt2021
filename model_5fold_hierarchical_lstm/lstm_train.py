...
# seed = 4000
# num_runs = 1
seeds = [65799, 61962, 15858]
n_epoch = 16
# for _ in range(num_runs):
for seed in seeds:
    # seed = random.randint(0, 10**5)
    # dir_rsl = f'results/16_epoch_trials/{seed}_{n_epoch}_epochs'
    ...
    for i in range(5):
        for j in range(5):
            ...
            dset_trn = AudioDataset(dir_mfcc, csv_info, 'TRN', comb=(i, j), seed=seed, get_label=get_label)
            dset_vld = AudioDataset(dir_mfcc, csv_info, 'VLD', comb=(i, j), seed=seed, get_label=get_label)
            dset_tst = AudioDataset(dir_mfcc, csv_info, 'TST', comb=(i, j), seed=seed, get_label=get_label)
            
            # initialize model
            model = Model_LSTM(n_concat=10, hidden_dim=64, device=device)
            
            # train model
            model.fit(dset_trn, dset_vld=dset_vld, n_epoch=n_epoch, b_size=4, lr=1e-3, save_model=False)
