...
class Model:
    ...
    def fit(self, dset_trn, dir_rsl, **kwargs):
        ...
        random_sampling_weight = kwargs.get('random_sampling_weight')
        ...
        elif random_sampling_weight is not None:
            sampling_weights = dset_trn.df_sampling_weights - 1

            adj_sampling_weights = [random_sampling_weight if s == 1 else 1\
                for s in sampling_weights]
            wrs = WeightedRandomSampler(adj_sampling_weights, replacement=replacement,
                num_samples=len(adj_sampling_weights))
        else:
            wrs = None
