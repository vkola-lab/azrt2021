...
class MFCC_Extractor():
    ...
    def __call__(self, arr):
        ...
        eng = self.backend.where(eng == 0, np.finfo(np.float32).eps,
            eng)  # numerical stability for log
        # step 3: apply Mel filter bank
        tmp = tmp @ self.bnk.T
        tmp = self.backend.where(tmp == 0, np.finfo(np.float32).eps,
            tmp)  # numerical stability for log
...
#            cp.get_default_memory_pool().free_all_blocks()
    ...
    def _strided_split(self, arr):
        ...
        for i, seq in enumerate(arr):
            rsl[i*seq_len:(i+1)*seq_len] = self.backend.lib.stride_tricks.as_strided(seq,
                shape=(seq_len, self.winlen), strides=(self.winstep*d_size, d_size))
        return rsl, seq_len
    ...
    def _mel_filter_bank(self):
        ...
        frq_mel =\
            (0, 2595 * np.log10(1 + (self.samplerate / 2) / 700))  # (low, high)
        mel_pts = np.linspace(frq_mel[0], frq_mel[1],
            self.nfilt + 2)    # equally spaced in Mel scale
