...
from data_csv import AudioDataset
...
for i in range(5):
    for j in range(5):
        ...
        tmp = f"{dir_rsl}/split_csvs"
        if not os.path.isdir(tmp):
            os.makedirs(f"{dir_rsl}/split_csvs") 
        dset_trn.df_dat.to_csv(f"{dir_rsl}/split_csvs/i({i})_j({j})_{seed}_TRN.csv", index=False)
        dset_vld.df_dat.to_csv(f"{dir_rsl}/split_csvs/i({i})_j({j})_{seed}_VLD.csv", index=False)
        dset_tst.df_dat.to_csv(f"{dir_rsl}/split_csvs/i({i})_j({j})_{seed}_TST.csv", index=False)
        # initialize model
